...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.22901009.svg)](https://doi.org/10.5281/zenodo.22901009)
...
SPI needs only `Date` and `Precipitation`; a two-column sheet is accepted as
is. Header matching ignores case, spaces, dots and underscores. Real Excel
...
subprocess.run([
    "Rscript", "scripts/calculate_spi.R",
    "--input", "precipitation.xlsx", "--sheet", "all",
], check=True)

monthly = pd.read_csv("output/monthly_spi_Lowland.csv", parse_dates=["Date"])
```
...
48 checks covering aggregation, calendar alignment, the coverage rule, PET,
the standardisation of both indices, the per-calendar-month fit, the
zero-inflation correction, the drought classification and input validation.

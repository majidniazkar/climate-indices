#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# climate-indices | Command line entry point for SDI
#
#   Rscript scripts/calculate_sdi.R --input discharge.xlsx --sheet all
#
# Needs only date and daily discharge. With --sheet all, every worksheet is
# processed and the output workbook carries one sheet per gauge.
#
...
ROOT <- find_repo_root()
for (f in c("climate_io.R", "monthly.R", "classify.R",
            "spi_index.R", "sdi_index.R")) {
  # spi_index.R supplies rolling_sum() and fit_gamma_thom(), shared with SDI.
...
option_list <- list(
  ...
  make_option(c("-s", "--scales"), type = "character", default = "1,3,6,9,12,24",
              help = "Comma-separated SDI accumulation scales in months [default %default]"),
  ...
  make_option("--distribution", type = "character", default = "log-normal",
              help = paste("Distribution fitted to the accumulated flow:",
                           "log-normal, gamma or normal. 'normal' is the plain",
                           "standardised anomaly [default %default]")),
  ...
  make_option("--no-zero-correction", action = "store_true", default = FALSE,
              ...
              help = paste("Do not treat zero-flow accumulations as a probability",
                           "mass. Zero-flow months then have no defined SDI")),
  make_option("--prefix", type = "character", default = "monthly_sdi",
...
parser <- OptionParser(
  ...
  usage = "Rscript scripts/calculate_sdi.R --input FILE [options]",
  description = paste0(
    "\nCompute multi-scale SDI from a workbook of Date / Discharge.\n",
...
if (!opt$distribution %in% SDI_DISTRIBUTIONS) {
  fail(sprintf("--distribution must be one of: %s",
               paste(SDI_DISTRIBUTIONS, collapse = ", ")))
}
...
for (sh in targets) {
  say("")
  say("[", sh, "]")
  daily <- read_climate_excel(opt$input, sheet = sh, fields = c("Date", "Discharge"))
  ...
  n_gap <- sum(is.na(monthly$Discharge))
  if (n_gap > 0L) {
    say("  gaps     : ", n_gap, " month(s) without usable discharge",
        if (freq == "daily") sprintf(" (min coverage %.0f%%)", 100 * opt$min_coverage) else "")
  }
  n_zero <- sum(monthly$Discharge == 0, na.rm = TRUE)
  if (n_zero > 0L) say("  dry      : ", n_zero, " month(s) with no flow at all")

  if (nrow(monthly) < 360) {
    warning(sprintf(paste("[%s] The record holds %d months (%.1f years). SDI is normally",
  ...
  res <- compute_sdi(discharge_ts, scales = scales, distribution = opt$distribution,
  ...
  say("  SDI      : scales ", paste(sub("^SDI_", "", names(res$values)), collapse = ", "),
      " | ", opt$distribution)
  if (length(res$zero_months) > 0L) {
    say("  zero flow: ", if (zero_correction) "mixed-distribution treatment" else "left undefined",
        " for calendar month(s) ",
        paste(sort(unique(unlist(res$zero_months))), collapse = ", "))
  }
...
sheets[["Metadata"]] <- data.frame(
  Field = c("generated_at", "input_file", "worksheets", "input_frequency",
            "min_coverage", "scales_requested", "distribution", "zero_handling",
            "reference_period", "accumulation", "r_version"),
  Value = c(format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
            ...
            opt$distribution,
            if (zero_correction) "zero flow carried as a probability mass" else "zero flow left undefined",

...
validate_ranges <- function(df) {
  ...
  if ("Discharge" %in% names(df)) {
    q <- df$Discharge[!is.na(df$Discharge)]
    if (length(q) == 0L) {
      stop("The Discharge column is entirely missing.", call. = FALSE)
    }
    if (any(q < 0)) {
      stop(sprintf("Discharge contains %d negative value(s); check the input file.",
                   sum(q < 0)), call. = FALSE)
    }
  }
